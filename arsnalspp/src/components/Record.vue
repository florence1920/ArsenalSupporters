        ...
        <div class="inner">
            <ul class="record">
                <li>
                    <ul>
                        <p>goal</p>
                        <li class="player_name" v-for="player in this.$store.state.top3GoalArr" v-bind:key="player.name">
                            <p> {{ player.name}} {{player.goal}} </p>
                        </li>
                    </ul>
                    <ul>
                        <p>assist</p>
                        <li class="player_name" v-for="player in this.$store.state.top3AssiArr" v-bind:key="player.name">
...
export default {
    ...
    created(){
        //Goal새로운 배열 만들기
        this.$store.commit('goalRecord');
        this.$store.commit('assistRecord');
